...
and then explicitly invoke that from the Python \inlinecode{run()} function. In
the example below we create an \actionx{} \inlinecode{CLOSEWELLS} which will
...
It is important that the dummy \actionx{} called from Python is not also run by
...
\begin{enumerate}
  ...
  \item If the \kw{CLOSEWELLS} action is invoked by mistake the condition should
